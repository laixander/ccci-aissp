            ...
            <div class="flex items-start justify-between">
                <div class="space-y-1">
                    <div class="flex items-center gap-3">
                        ...
                        <UBadge :label="status" :color="status === 'Active' ? 'success' : 'neutral'" variant="soft"
                            class="rounded-full" />
                    </div>
                ...
                <div class="flex items-center space-x-2">
                    <UTooltip text="Edit Template">
                        <UButton icon="i-lucide-square-pen" color="neutral" variant="ghost" size="sm" @click="$emit('edit')" />
                    </UTooltip>
                    <UTooltip text="Duplicate Template">
                        <UButton icon="i-lucide-copy" color="neutral" variant="ghost" size="sm" @click="$emit('copy')" />
                    </UTooltip>
                    ...
                    <Confirmation @delete="$emit('delete')">
                        ...
                        <template #message>
                            <p class="text-gray-800 dark:text-gray-100 font-medium">
                                Are you sure you want to delete this <span class="font-extrabold text-primary">{{
                                    title }}</span> section?
                            </p>
                        </template>
                    </Confirmation>
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <UBadge v-for="(section, index) in sections" :key="index" :label="section" variant="soft"
                            color="neutral" class="rounded-full" />
                    </div>
                </div>
                <UBadge :label="`${sections.length} sections`" variant="outline" color="neutral" class="rounded-full" />
            </div>
